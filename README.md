# 허도경(HEO DO KYUNG) 마크업 / CSS 가이드

본 마크업 가이드는 프로젝트 진행시 동일한 산출물을 작업하는데 주 목적을 두고 있습니다. 
일관된 코드의 산출물은 운영(유지보수)의 용이성과 코드의 재사용, 간결한 코드가 유지되는 데 도움을 줍니다. 

해당 가이드는 [Code Guide by @mdo](http://mdo.github.io/code-guide)를 기반으로 작성하였으며 프로젝트를 진행하면서 개선이 필요하다고 생각한 부분을 첨부,첨삭 하였습니다.
...
* 모든 엘리먼트 명과 애트리뷰트 명은 BEM 방법론을 기준으로 한 작성을 원칙으로 합니다.
    * CSS의 모든 속성은 BEM 방법론으로 작업되며 가독성 및 동일한 산출물이 나올수 있도록 합니다..
* ID는 사용하지 않으며 Class로만 작성하고 레이아웃 역시 포함됩니다. (예: `<div class="header" />`, `<div class="footer" />`, `<div class="container" />`, `<div class="gnb" />` 등)
